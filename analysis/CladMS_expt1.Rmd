...
```{r fig.height=8, fig.width=8, warning=F, message=F, results='hide'}
...
```{r}
pp_check(model_rust, nsamples = 1000)
```
...
```{r, warning=FALSE}
describe_posterior(model_rust, test = c("p_direction", "rope"))
```
...
```{r, fig.height=8, fig.width=8, warning=F, message=F, results='hide'}
#scale variables for logistic model (sensu Genlam et al 2008)
dat %<>%
  mutate(
    disease_severity_sc = scale(disease_severity, scale=sd(disease_severity)*2),
    cladeHerbarum = case_when(
      clade=="herbarum" ~ 1,
      T ~ 0
    ) %>% scale(scale=F),
    regionWest = case_when(
      region=="West" ~ 1,
      T ~ 0
    ) %>% scale(scale=F)
  )
...
model_myco <- brm(
  mycoparasitism ~ disease_severity_sc + cladeHerbarum + regionWest + 
    (1|plate) + (1|spp) +
    (1|gr(id, cov = A)),
  data = dat, 
  family = bernoulli(), 
...
```{r}
pp_check(model_myco, nsamples=1000)
```
...
Finally, we can examine the evidence for our (fixed) effects of interest from the posterior distribution
```{r, warning=FALSE}
describe_posterior(model_myco, test = c("p_direction", "rope"))
```
