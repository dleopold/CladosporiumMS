...
We found no evidence (BF = `r round(exp(post_myco$log_BF[4]),2)`) that *Cladosporium* isolates differed, on average, in their likelihood of displaying a mycoparasitism phenotype based on whether they originated from east (`r round(em_region[1,2])`; 89% HDI: `r em_region[1,3:4] %>% round(2) %>% paste(.,sep="",collapse="-")`) or west (`r round(em_region[2,2])`; 89% HDI: `r em_region[2,3:4] %>% round(2) %>% paste(.,sep="",collapse="-")`) of the Cascades. 
